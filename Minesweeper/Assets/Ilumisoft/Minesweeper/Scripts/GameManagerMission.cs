...
public class GameManagerMission : MonoBehaviour, ITileClickListener
{
    [SerializeField]
        ...
        public void OnTileClick(Tile tile)
        {
            ...
            {
                TileRevealerMission tileRevealer = new TileRevealerMission(grid);
        ...
        private void LockAllTheTiles()
        {
            for (int y = 0; y < grid.Height; y++)
            {
                for (int x = 0; x < grid.Width; x++)
                {
                    if (grid.TryGetTile(x, y, out Tile tile) == true)
                    {
                        tile.SwitchLock();
                    }
                }
